...
namespace Logic.Units.Allies
{
    public class Grasshopper:Unit
   {
            ...
            : base(hp, armor, strength, range, cost, buildingTime, model, atackInterval)
    {
        ...
        public Grasshopper(LoadModel model,List<InteractiveModel>ants):base(model)
        {
            sfereModel=new InteractiveModel(new LoadModel(StaticHelpers.StaticHelper.Content.Load<Model>("Models/grasshopper"),model.Position,Vector3.Zero,Vector3.One,StaticHelpers.StaticHelper.Device,model.light));
            Scope = 100;
            ArmorBuffValue = 100;
            this.Ants = ants;
